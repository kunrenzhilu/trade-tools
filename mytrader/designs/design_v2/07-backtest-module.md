...
### 10.4.1 Top-K 选择三级 Fallback（迭代 #9 新增，迭代 #11 前置健全性门槛，迭代 #12 前置 alpha gate，迭代 #16 放宽 alpha gate 阈值）
...
- `_optimize_ensemble_weights` 保持 `max(a, 0.0)` 不变（负 alpha 在多策略 ensemble 中权重仍为 0，保守设计）
