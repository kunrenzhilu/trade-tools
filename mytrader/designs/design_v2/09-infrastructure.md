...
system:
  ...
  trading_timezone: "America/New_York"  # 调度/交易逻辑时区（Job 以此为准）
  log_level: "INFO"
  # 时区策略：日志 timestamp 统一 UTC 存储；显示时转换为 system.timezone
  #           告警消息同时附带 ET 时间（便于对照交易时间，避免复盘时间错位）
...
strategy_matrix:
  ...
  signal_valid_bars: 3           # 事件型信号的有效期（bar 数），防止漏掉趋势中段

signal_ranker:
  top_k: 5
  candidates_multiplier: 2       # 输出 top_k × multiplier 候选供 Risk Manager 递补
...
risk:
  ...
  # 隔夜风险管理（可选，Phase 5 初期先启用 earnings_exit）
  earnings_exit_days_before: 1  # 财报前 T-1 强制平仓
  vix_high_threshold: 30        # VIX 超此值时 max_single_position_pct 降至 10%
  circuit_breaker:
